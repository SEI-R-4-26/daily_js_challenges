...
function reduceArray(arr, callback, val) {
  ...
  arr.forEach((element, index) => {
    if (count === 0) {
      count++
      console.log(arr, callback, callback(val), val, element, index)
      callback(val, callback, index)
    } else {
      console.log(arr, callback, val, element, index)
      return callback(val, element, index)
    }
  })
  return callback
}
