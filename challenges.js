...
function formatWithPadding(num, str, pad) {
  let padding = parseInt(pad) - parseInt(num.toString().length)
  if (padding <= 0) {
    return num.toString()
  }
  let paddedStr = ''
...
function isPalindrome(str) {
  if (
    ...
    str.toUpperCase().split(' ').join('').split('').join('')
  ) {
    return true
  }
  return false
...
function fromPairs(arr) {
  let obj = {}
  arr.forEach((array) => (obj[array[0]] = array[1]))
  return obj
}
/*-----------------------------------------------------------------
...
function mergeObjects(obj, ...objects) {
...
function mapArray(arr, func) {
  let newArr = []
  arr.forEach((item, index) => newArr.push(func(item, index)))
  return newArr
}
/*-----------------------------------------------------------------
