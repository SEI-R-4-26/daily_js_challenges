...
const balancedBrackets = (bracketStr) => {
  let balanceCheck = true
  arr = bracketStr.split('')
  while (balanceCheck === true && arr.length > 0) {
    balanceCheck = false
...
const isWinningTicket = (ary) => {
  for (let i = 0; i < ary.length; i++) {
    let testChar = String.fromCharCode(ary[i][1])
    let testArr = ary[i][0].split('')
    if (testArr.findIndex((element) => element === testChar) < 0) {
      return false
    }
  }
  return true
